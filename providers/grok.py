...
class GrokProvider(BaseProvider):
    ...
    async def get_response(self) -> str:
        try:
            await self._page.wait_for_selector('[aria-label*="Stop"], .loading', timeout=15000)
        except Exception:
            pass
        try:
            await self._page.wait_for_selector('[aria-label*="Stop"], .loading', state="hidden", timeout=120000)
        except Exception:
            pass

        await asyncio.sleep(0.5)

        messages = await self._page.locator('[class*="message"][class*="assistant"], [data-role="assistant"]').all()
        if messages:
